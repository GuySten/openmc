...
class ContinuousTabular(EnergyDistribution):
    ...
    def from_ace(cls, ace, idx, ldis):
        ...
        for i in range(n_energy_in):
            ...
            # Create continuous distribution. A negative tabulated density is
            # unphysical and makes the linear-linear inversion in the sampler
            # return a meaningless outgoing energy, so clamp it -- but say so,
            # rather than passing it through the way ignore_negative alone
            # would.
            p_continuous = data[1][n_discrete_lines:]/EV_PER_MEV
            n_negative = int(np.count_nonzero(p_continuous < 0.0))
            if n_negative > 0:
                warn(f'Outgoing energy distribution in {ace.name} has '
                     f'{n_negative} negative probability density value(s), '
                     'which have been set to zero.')
                p_continuous = np.maximum(p_continuous, 0.0)
