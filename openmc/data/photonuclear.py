...
_REACTION_NAME.update({i: "(g,n{})".format(i - 50) for i in range(50, 91)})
_REACTION_NAME.update({i: "(g,d{})".format(i - 650) for i in range(650, 699)})
_REACTION_NAME.update({i: "(g,t{})".format(i - 700) for i in range(700, 749)})
_REACTION_NAME.update({i: "(g,3He{})".format(i - 750) for i in range(750, 799)})
_REACTION_NAME.update({i: "(g,a{})".format(i - 800) for i in range(800, 849)})
_REACTION_NAME.update({i: "(g,2n{})".format(i - 875) for i in range(875, 891)})
...
class PhotonuclearReaction(EqualityMixin):
    ...
    def __repr__(self):
        if self.mt in _REACTION_NAME:
            return "<photo-nuclear Reaction: MT={} {}>".format(
                self.mt, _REACTION_NAME[self.mt]
            )
        else:
            return "<photo-nuclear Reaction: MT={}>".format(self.mt)
    ...
    def from_endf(cls, ev, mt):
        """Generate photo-nuclear reaction from an ENDF evaluation
        ...
        Returns
        -------
        openmc.data.PhotonuclearReaction
    ...
    def to_hdf5(self, group, energy):
        """Write photo-nuclear reaction to an HDF5 group
    ...
    def from_hdf5(cls, group, energy):
        """Generate photo-nuclear reaction from an HDF5 group
...
class Photonuclear(EqualityMixin):
    ...
    def __repr__(self):
        return "<Photonuclear: {}>".format(self.name)
    ...
    def from_ace(cls, ace_or_filename, metastable_scheme="nndc"):
        ...
        if np.count_nonzero(absorption_xs) > 0:
            absorption = PhotonuclearReaction(101)
