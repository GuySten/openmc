...
void initialize_mpi(MPI_Comm intracomm)
{
  ...
  MPI_Get_address(&b.super_gen, &disp[11]);
  MPI_Get_address(&b.wgt_born, &disp[12]);
  MPI_Get_address(&b.wgt_super, &disp[13]);
  MPI_Get_address(&b.wgt_ww_born, &disp[14]);
  MPI_Get_address(&b.n_split, &disp[15]);
  MPI_Get_address(&b.n_collision, &disp[16]);
  for (int i = 16; i >= 0; --i) {
    disp[i] -= disp[0];
  }
