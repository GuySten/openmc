...
TEST_CASE("Test log1pmx")
{
  using Catch::Matchers::WithinRel;
  using openmc::log1pmx;

  // Reference values from mpmath at 60 digits: mp.log(1+x) - x
  std::vector<std::pair<double, double>> ref {
    {-1.0e-12, -5.0000000000033332e-25},
    {-1.0e-08, -5.0000000333333335e-17},
    {-1.0e-04, -5.0003333583353341e-09},
    {-1.0e-02, -5.0335853501441186e-05},
    {-5.0e-02, -1.2932943875505337e-03},
    {-2.0e-01, -2.3143551314209757e-02},
    {-5.0e-01, -1.9314718055994531e-01},
    {-7.0e-01, -5.0397280432593594e-01},
    {-8.0e-01, -8.0943791243410046e-01},
    {-9.9e-01, -3.6151701859880898e+00},
    {1.0e-12, -4.9999999999966669e-25},
    {1.0e-04, -4.9996666916646670e-09},
    {1.0e-02, -4.9669146831917157e-05},
    {5.0e-01, -9.4534891891835615e-02},
    {1.0e+00, -3.0685281944005466e-01},
    {2.0e+00, -9.0138771133189044e-01},
    {1.0e+01, -7.6021047272016293e+00},
  };
  for (const auto& [x, expected] : ref) {
    REQUIRE_THAT(log1pmx(x), WithinRel(expected, 1e-14));
  }

  // The point of the function: the naive difference is hopeless near zero,
  // where the result is -x^2/2 and both terms are x.
